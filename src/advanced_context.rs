...
impl AdvancedContextManager {
    ...
    pub async fn initialize_workspace(&mut self, workspace_path: &str) -> Result<String, Box<dyn std::error::Error>> {
        let workspace_id = self.generate_workspace_id(workspace_path);
        self.current_workspace = Some(workspace_id.clone());
        
        let mut context = self.load_or_create_workspace(&workspace_id, workspace_path).await?;
    ...
    async fn load_or_create_workspace(&self, workspace_id: &str, workspace_path: &str) -> Result<WorkspaceContext, Box<dyn std::error::Error>> {
        ...
        if Path::new(&context_path).exists() {
            ...
            Ok(self.create_default_workspace(workspace_id, workspace_path).await?)
        }
    }

    async fn create_default_workspace(&self, workspace_id: &str, workspace_path: &str) -> Result<WorkspaceContext, Box<dyn std::error::Error>> {
    ...
    async fn detect_project_config(&self, workspace_path: &str) -> Result<ProjectConfig, Box<dyn std::error::Error>> {
    ...
    fn parse_cargo_config(&self, content: &str) -> ProjectConfig {
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with("name = ") {
                name = trimmed.split('=').nth(1).unwrap_or("").trim().trim_matches('"').to_string();
            } else if trimmed.starts_with("version = ") {
                version = trimmed.split('=').nth(1).unwrap_or("").trim().trim_matches('"').to_string();
    ...
    fn parse_package_json(&self, content: &str) -> ProjectConfig {
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            let name = json.get("name").and_then(|v| v.as_str()).unwrap_or("Node.js Project").to_string();
    ...
    async fn create_session_data(&self, workspace_path: &str) -> Result<SessionData, Box<dyn std::error::Error>> {
    ...
    async fn get_git_branch(&self, workspace_path: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    async fn get_git_status(&self, workspace_path: &str) -> Result<String, Box<dyn std::error::Error>> {
        let output = std::process::Command::new("git")
    ...
    #[allow(dead_code)]
    pub async fn add_conversation_message(&mut self, role: &str, content: &str, file_refs: Vec<String>, command: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(workspace_id) = &self.current_workspace {
            ...
            let thread = context.conversation_threads.entry(thread_id.clone()).or_insert_with(|| {
                ConversationThread {
            ...
            if thread.messages.len() > self.memory_limit {
                thread.messages.drain(0..thread.messages.len() - self.memory_limit);
            }
    ...
    #[allow(dead_code)]
    pub async fn get_relevant_context(&self, query: &str, max_messages: usize) -> Result<Vec<ContextMessage>, Box<dyn std::error::Error>> {
        if let Some(workspace_id) = &self.current_workspace {
    ...
    #[allow(dead_code)]
    pub async fn track_file_change(&mut self, file_path: &str, change_type: &str, summary: &str) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(workspace_id) = &self.current_workspace {
            let mut context = self.load_or_create_workspace(workspace_id, "").await?;
            ...
            if context.recent_changes.len() > 100 {
                context.recent_changes.drain(0..context.recent_changes.len() - 100);
            }
    ...
    pub async fn get_workspace_summary(&self) -> Result<String, Box<dyn std::error::Error>> {
        if let Some(workspace_id) = &self.current_workspace {
            ...
            let total_messages: usize = context.conversation_threads.values()
                .map(|t| t.messages.len())
                .sum();
            
            let recent_files: Vec<String> = context.recent_changes.iter()
                .take(5)
            ...
            Ok(format!(
                ...
                recent_files.iter().map(|f| format!("- {}", f)).collect::<Vec<_>>().join("\n")
            ))
        } else {
    ...
    async fn save_workspace_context(&self, context: &WorkspaceContext) -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[allow(dead_code)]
    pub async fn cleanup_old_contexts(&self, days_old: u64) -> Result<usize, Box<dyn std::error::Error>> {
        let mut cleaned = 0;
