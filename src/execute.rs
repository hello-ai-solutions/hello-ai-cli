        ...
        if self.command.starts_with("docker ") {
            ...
            if let Ok(output) = docker_check {
                if !output.status.success() {
                    return Err("Docker daemon not running. Please start Docker Desktop and try again.".into());
                }
            }
        }

        // Check for interactive commands that shouldn't be executed directly
        let interactive_commands = ["pwsh", "powershell", "python", "node", "irb", "mysql", "psql", "mongo", "redis-cli"];
