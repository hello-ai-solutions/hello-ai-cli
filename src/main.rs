...
use rich_interface::{RichInterface, AmazonQTheme};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Some(Commands::Chat { input, no_interactive, resume, agent, model, trust_all_tools, trust_tools, wrap, verbose }) => {
            if no_interactive {
                if let Some(msg) = input {
                    run_single_chat_with_options(msg, resume, agent, model, trust_all_tools, trust_tools, wrap, verbose, app_config.clone()).await?;
                } else {
                    eprintln!("Error: Input required for non-interactive mode");
                    std::process::exit(1);
                }
            } else {
                run_interactive_chat_with_options(resume, agent, model, trust_all_tools, trust_tools, wrap, verbose, app_config.clone()).await?;
            }
        }
...
async fn run_interactive_chat(mut app_config: Config) -> Result<(), Box<dyn std::error::Error>> {
                ...
                match sig {
                    SIGINT => {
                        ...
                        *count += 1;
                        match *count {
                            1 => {
                                println!("\n{} {} {}", "⚠️".yellow(), "Ctrl+C pressed once.".bright_yellow(), "Press 2 more times to exit.".bright_black());
                            }
                            2 => println!("\n{} {} {}", "⚠️".red(), "Ctrl+C pressed twice.".bright_red(), "Press once more to force exit.".bright_black()),
                            _ => {
                                println!("\n{} {}", "💥".red(), "Force exit after 3 Ctrl+C presses.".bright_red());
                                std::process::exit(1);
                            }
                        }
                    }
    ...
    let comprehend_pii = if app_config.security.pii_scanner && app_config.cloud.provider == "aws" {
        match ComprehendPiiScanner::new().await {
            ...
            Err(e) => {
                println!("{} Failed to initialize AWS Comprehend PII: {}", "⚠️".yellow(), e);
                None
            }
        }
    } else {
    ...
    let local_pii = if app_config.security.pii_scanner {
        let pii_model = app_config.security.pii_model
            .as_ref()
    ...
    print!("{}", rich_interface.format_startup_banner(nightfall_enabled, &app_config));
    ...
    println!("  {} {}", "•".truecolor(255, 153, 0), AmazonQTheme::format_muted("Type your questions naturally"));
    println!("  {} {}", "•".truecolor(255, 153, 0), AmazonQTheme::format_muted("Commands are automatically detected and executed"));
    println!("  {} {}", "•".truecolor(255, 153, 0), AmazonQTheme::format_muted("Use 'y' to confirm risky commands, 'a' to confirm all"));
    println!("  {} {}", "•".truecolor(255, 153, 0), AmazonQTheme::format_muted("Type 'quit', '/quit', or press Ctrl+C 3 times to exit"));
    println!("  {} {}", "•".truecolor(255, 153, 0), AmazonQTheme::format_muted("Press Ctrl+C 3 times to force exit"));
    println!();
    ...
    loop {
        ...
        if input == "quit" || input == "/quit" {
            println!("{}", AmazonQTheme::format_info("Goodbye! Thank you for using Hans AI LLM CLI."));
        ...
        if app_config.features.orchestrate_mode && 
           (input.contains("orchestrate") || input.contains("manage agents") || 
            input.contains("security best practices") || input.contains("microservice dependency") ||
            input.contains("analyze logs") || input.contains("troubleshoot")) {
            ...
            match orchestrator.execute_orchestrated_task(&input, &app_config).await {
                Ok(result) => {
                    println!("{}", result);
                }
                Err(e) => {
                    println!("{} {}", "Orchestrator Error:".bright_red().bold(), e.to_string().red());
                }
            }
            continue;
        }
            ...
            match command_part {
                "/agents" => {
                    ...
                    if args.len() == 1 {
                        ...
                        println!("     /agents config              - Show current agent configurations");
                        ...
                        if args.len() == 2 {
                            ...
                            match multi_agent.configure_agent(agent_name, provider, model) {
                                Ok(_) => {
                                    println!("✅ Configured {} to use {} ({})", agent_name, provider, model);
                                ...
                                Err(e) => {
                                    println!("❌ Error: {}", e);
                                    println!("Available agents: {}", multi_agent.get_available_agents().join(", "));
                                }
                            }
                        } else {
                ...
                "/test" => {
                    ...
                    if args.len() == 1 {
                        ...
                        println!("  /test layout-format - Run layout and formatting validation tests");
                        continue;
                    }
                ...
                "/verbosity" => {
                    ...
                    if parts.len() == 1 {
                        println!("Current verbosity: {}", app_config.features.verbosity.bright_cyan());
                        println!("Available levels:");
                        println!("  • {} - Show LLM responses + all system messages", "full".bright_green());
                        println!("  • {} - Show LLM responses + essential messages only", "minimal".bright_yellow());
                        println!("  • {} - Show LLM responses + commands only", "commands-only".bright_red());
                        ...
                        match parts[1] {
                            ...
                            _ => {
                                println!("Invalid verbosity level. Use: full, minimal, or commands-only");
                            }
                        }
                    }
                    continue;
                ...
                "/editor" => {
                    if let Some(editor_input) = handle_editor_prompt().await {
                        ...
                        println!("{} {}", ">".bright_green().bold(), editor_input.bright_white());
                ...
                "/reply" => {
                    if let Some(reply_input) = handle_reply_prompt(&conversation_history).await {
                        println!("{} {}", ">".bright_green().bold(), reply_input.bright_white());
                ...
                "/s3logs" => {
                    println!("📤 S3 Logs - Use: /s3logs-enable <bucket>, /s3logs-status, /s3logs-test");
                    continue;
                }
                "/s3logs-enable" => {
                ...
                "/s3logs-enable-bucket" => {
                    ...
                    match s3_logs::S3Logger::save_config(&config).await {
                        Ok(_) => println!("✅ S3 logging enabled: s3://{}/{}", config.bucket, config.prefix),
                ...
                "/stream-test" => {
                    ...
                    let test_conversation = vec![
                        json!({"role": "user", "content": "Say hello"})
                    ];
                    
                    match streaming_client.stream_response(&test_conversation, None).await {
                ...
                "/security-scan" => {
                    ...
                    match std::fs::metadata(target) {
                        ...
                        Ok(_) => {
                            match scanner.scan_file(target).await {
                                Ok(issues) => {
                                    if issues.is_empty() {
                                        println!("✅ No security issues found");
                                    } else {
                                        for issue in issues {
                                            println!("⚠️  {} (Line {}): {}", issue.severity, issue.line, issue.description);
                ...
                "/explain" => {
                    ...
                    match std::fs::read_to_string(file_path) {
                        Ok(content) => {
                            let language = match std::path::Path::new(file_path).extension().and_then(|s| s.to_str()) {
                                Some("rs") => "rust",
                                Some("py") => "python",
                                Some("js") => "javascript",
                                _ => "unknown",
                    ...
                    match generator.generate_tests_for_file(file_path).await {
                        Ok(test_code) => {
                            ...
                            match generator.create_test_file(file_path, &test_code).await {
                                Ok(test_file_path) => println!("✅ Test file created: {}", test_file_path),
                        ...
                        match args[1] {
                            ...
                            "status" => {
                                println!("Orchestrate mode: {}", 
                                    if app_config.features.orchestrate_mode { "enabled".green() } else { "disabled".red() });
                            }
                            _ => {
                ...
                _ => {
                    if input.starts_with("/save ") {
                        ...
                        handle_save_conversation(&conversation_history, Some(name.to_string())).await;
                        continue;
                    }
                    println!("{} Unknown command: {}", "Error:".bright_red(), input.bright_yellow());
                    continue;
                }
            }
        }
        ...
        if cloud_logging::should_log(&app_config.logging, "info") {
            let _ = cloud_logging::log_message(&app_config.logging, "info", &format!("User input: {}", input)).await;
        }
        ...
        let (final_input, pii_detected) = if app_config.security.pii_scanner {
            if let Some(ref scanner) = comprehend_pii {
                ...
                println!("{} Scanning input with Amazon Comprehend PII...", "🔍".bright_black());
                match scanner.scan_text(&input).await {
                    Ok(result) => {
                    if result.has_pii {
                        println!("{} PII detected and sanitized", "🔒".yellow());
                        for entity in &result.entities {
                            println!("  - {}: {:.1}% confidence", entity.entity_type, entity.confidence * 100.0);
                        }
                ...
                println!("{} PII scanner not available (check Ollama service)", "⚠️".yellow());
                (input.clone(), false)
            }
        } else {
            (input.clone(), false)
                    ...
                    Ok(nightfall_pii_detected) => {
                        ...
                        if !simple_pii_scan && nightfall_pii_detected {
                            ...
                            if let Ok(nightfall_result) = nightfall.scan_text(&input, "user input").await {
                                _nightfall_redacted_input = nightfall.redact_nightfall_findings(&input, &nightfall_result.findings);
                            }
                        }
                    }
                    Err(e) => {
                        eprintln!("{} Nightfall scan failed: {}", "⚠️".yellow(), e.to_string().yellow());
                    }
                }
            }
        }
        ...
        if simple_pii_scan {
            ...
            if pii_detected {
                ...
                print!("{}", rich_interface.format_pii_detection(true, &pii_details, false));
            }
            ...
            if let Some(ref nightfall) = nightfall_client {
                if let Ok(nightfall_result) = nightfall.scan_text(&input, "user input").await {
                    if !nightfall_result.findings.is_empty() {
                        ...
                        print!("{}", rich_interface.format_pii_detection(true, &nightfall_details, true));
                    }
                }
            }
        }
        ...
        if pii_detected {
            if let Err(e) = ses_alerts.send_pii_alert(
                &[],  // Empty findings array since we're using Comprehend
                "user input",
                &input,
                Some("Amazon Comprehend detected PII in user input"),
            ).await {
                eprintln!("{} Failed to send PII alert: {}", "⚠️".yellow(), e.to_string().yellow());
            }
        }
        ...
        let _os_context = format!("System Context: Running on {} ({}). ", system_context.host_os, system_context.host_arch);
        ...
        let response_result = if app_config.model.use_local_llm {
            ...
            streaming_client.stream_response(&conversation_history, Some(&system_prompt)).await
        } else {
            ...
            match app_config.cloud.provider.as_str() {
                ...
                _ => {
                    let user_message = conversation_history.last()
        ...
        match response_result {
            Ok(response_text) => {
                ...
                if let Some((action_header, user_message)) = action_header::parse_action_header(&sanitized_ai_response) {
                    ...
                    for create_action in &action_header.create {
                        ...
                        if let Some(parent) = std::path::Path::new(&create_action.path).parent() {
                            if let Err(e) = std::fs::create_dir_all(parent) {
                                println!("{}", render_msg(MsgKind::Error, &format!("Failed to create directory {}: {}", parent.display(), e)));
                                failed_commands.push((format!("mkdir -p {}", parent.display()), e.to_string()));
                                continue;
                            }
                        }
                        ...
                        if let Err(e) = std::fs::write(&create_action.path, &create_action.content) {
                            println!("{}", render_msg(MsgKind::Error, &format!("Failed to create file {}: {}", create_action.path, e)));
                            failed_commands.push((format!("create file {}", create_action.path), e.to_string()));
                        }
                    }
                    ...
                    for run_action in &action_header.run {
                        ...
                        println!("{} {} {}", risk_indicator, "Command:".bright_blue(), run_action.cmd);
                ...
                if app_config.features.execution_mode != "chatbot" {
                    ...
                    for tool in tools {
                        match tool {
                            ToolCall::Execute(cmd) => {
                                ...
                                println!("{} {} {}", ">".bright_green().bold(), risk_indicator, cmd.command.bright_white());
                                
                                if requires_confirmation && !confirm_all {
                                    let confirmation_prompt = format!("{} ", "⚠️  Confirm execution? (y/N/a for all):".bright_yellow().bold());
                                    ...
                                    if confirmation.starts_with('a') {
                                        confirm_all = true;
                                        println!("{}", "✅ Confirming all remaining commands".bright_green());
                                ...
                                if !command_guard.should_execute(&cmd.command) {
                                    println!("{}", "⏭️  Skipping duplicate command (executed recently)".bright_yellow());
                                    continue;
                                }
                                ...
                                match cmd.execute().await {
                                    Ok(output) => {
                                        if !output.trim().is_empty() {
                                            ...
                                            all_outputs.push((format!("execute: {}", cmd.command), output.clone()));
                                            
                                            // Individual command analysis (guard agent)
                                            if app_config.analysis.command_analysis {
                                                match analyze_command_output(&client, &cmd.command, &output).await {
                                                    Ok(analysis) => {
                                    ...
                                    Err(e) => {
                                        println!("{}", render_msg(MsgKind::Error, &e.to_string()));
                                        failed_commands.push((cmd.command.clone(), e.to_string()));
                                    }
                                }
                            }
                            ...
                            ToolCall::UseAws(use_aws) => {
                                match use_aws.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                            ...
                            ToolCall::UseAzure(use_azure) => {
                                match use_azure.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                            ...
                            ToolCall::UseGcp(use_gcp) => {
                                match use_gcp.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                            ...
                            ToolCall::Knowledge(knowledge) => {
                                match knowledge.execute().await {
                                    Ok(output) => {
                                        if !output.trim().is_empty() {
                                            println!("{}", output);
                                        }
                            ...
                            ToolCall::TodoList(todo_list) => {
                                match todo_list.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                            ...
                            ToolCall::Thinking(thinking) => {
                                match thinking.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                            ...
                            ToolCall::OrchestratedThinking(orchestrated_thinking) => {
                                match orchestrated_thinking.execute().await {
                                    Ok(output) => {
                                        println!("{}", output);
                                        all_outputs.push(("orchestrated_thinking".to_string(), output));
                                    }
                                    Err(e) => {
                                        println!("{} {}", "Error:".bright_red().bold(), e.to_string().red());
                                    }
                    ...
                    if app_config.security.guard_rail_enabled {
                        ...
                        if should_validate {
                            ...
                            let validation_result = if app_config.security.guard_rail_provider == "local" {
                                // Use local LLM for validation
                                let streaming_client = StreamingClient::new_local(
                                    app_config.model.local_llm_endpoint.clone(),
                                    app_config.model.local_llm_model.clone()
                                );
                                streaming_client.stream_response(&[
                                    json!({
                                        "role": "user",
                            ...
                            match validation_result {
                                Ok(validation_result) => {
                                    if !validation_result.trim().eq_ignore_ascii_case("VALIDATED") {
                    ...
                    if !failed_commands.is_empty() {
                        let failure_summary = failed_commands.iter()
                        ...
                        match retry_result {
                            Ok(retry_suggestions) => {
                                println!("\n{} {}", "🛡️".truecolor(184, 134, 11), "Alternative suggestions:".truecolor(184, 134, 11).bold());
                            ...
                            Err(e) => {
                                println!("{} Failed to get retry suggestions: {}", "⚠️".yellow(), e);
                            }
                        }
                    }
                    ...
                    if !all_outputs.is_empty() {
                        let combined_results = all_outputs.iter()
                        ...
                        if !combined_results.trim().is_empty() {
                            ...
                            let should_analyze = app_config.features.auto_analysis.unwrap_or(false) && (
                            ...
                            if should_analyze {
                                let analysis_request = format!("Analyze these results and provide a brief summary:\n\n{}", combined_results);
                                
                                match get_ai_response(&client, &analysis_request, &app_config).await {
                                    Ok(analysis) => {
                                        if !analysis.trim().is_empty() {
                                            let formatted = rich_interface.format_markdown_response(&analysis);
            ...
            Err(_e) => {
                ...
                println!("{}", "🔄 Falling back to synchronous response...".bright_yellow());
...
async fn analyze_command_output(client: &BedrockClient, command: &str, output: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if output.contains("Cannot connect to the Docker daemon") || 
       ...
       output.contains("docker.sock") {
        return Ok("Docker daemon not running. Start Docker Desktop to resolve this issue.".to_string());
    }
...
fn determine_analysis_context(command: &str, output: &str) -> AnalysisContext {
    ...
    if output.contains("github") || output.contains("repository") || command.contains("github") {
        AnalysisContext {
            ...
            focus_areas: vec!["code quality".to_string(), "community health".to_string(), "development activity".to_string()],
        ...
        AnalysisContext {
            ...
            focus_areas: vec!["error resolution".to_string(), "root causes".to_string(), "quick fixes".to_string()],
            header_emoji: "🔧".to_string(),
        ...
        AnalysisContext {
            ...
            focus_areas: vec!["resource optimization".to_string(), "security".to_string(), "cost efficiency".to_string()],
        ...
        AnalysisContext {
            ...
            style: "development-focused".to_string(),
            focus_areas: vec!["build health".to_string(), "dependencies".to_string(), "performance".to_string()],
            header_emoji: "🦀".to_string(),
        ...
        AnalysisContext {
            ...
            focus_areas: vec!["system overview".to_string(), "key patterns".to_string(), "strategic insights".to_string()],
            header_emoji: "📊".to_string(),
        ...
        AnalysisContext {
            ...
            focus_areas: vec!["immediate insights".to_string(), "actionable items".to_string(), "next steps".to_string()],
            header_emoji: "💡".to_string(),
...
async fn get_local_llm_response(endpoint: &str, model: &str, message: &str) -> Result<String, Box<dyn std::error::Error>> {
...
async fn get_ai_response(client: &BedrockClient, message: &str, app_config: &Config) -> Result<String, Box<dyn std::error::Error>> {
    // Log AI request
    if cloud_logging::should_log(&app_config.logging, "debug") {
        let _ = cloud_logging::log_message(&app_config.logging, "debug", &format!("AI request: {}", message)).await;
    }
    ...
    match unified_cloud_providers::get_cloud_llm_response(&app_config.cloud, message).await {
        Ok(response) => {
            // Log AI response
            if cloud_logging::should_log(&app_config.logging, "debug") {
                let _ = cloud_logging::log_message(&app_config.logging, "debug", &format!("AI response: {}", response)).await;
            }
            return Ok(response);
        },
        Err(e) => {
            if cloud_logging::should_log(&app_config.logging, "warn") {
                let _ = cloud_logging::log_message(&app_config.logging, "warn", &format!("Cloud provider failed: {}", e)).await;
            }
            println!("{} Cloud provider failed, trying fallback: {}", "⚠️".yellow(), e);
        }
    }
    ...
    match app_config.model.provider.as_str() {
        "local" | _ if app_config.model.use_local_llm => {
            return get_local_llm_response(&app_config.model.local_llm_endpoint, &app_config.model.local_llm_model, message).await;
        }
        "openai" => {
...
fn extract_tool_calls(text: &str) -> Option<Vec<ToolCall>> {
    ...
    else if text.contains("read file") || text.contains("show me") || text.contains("list") || 
       text.contains("directory") || text.contains("what's in") || text.contains("files") ||
       text.contains("contents") || text.contains("examine") {
        tools.push(ToolCall::FsRead(FsRead {
            operations: vec![fs_read::FsReadOperation::Directory(fs_read::FsDirectory {
                path: ".".to_string(),
                depth: 1,
    ...
    if text.contains("create file") || text.contains("write file") || text.contains("save to") ||
       text.contains("write to") || text.contains("create a file") || text.contains("make a file") ||
       text.contains("generate file") || text.contains("output to file") {
    ...
    if text.contains("aws ") || text.contains("AWS ") || text.contains("s3 ") || text.contains("ec2 ") ||
       text.contains("lambda ") || text.contains("cloudformation") || text.contains("describe ") ||
       text.contains("list ") && (text.contains("bucket") || text.contains("instance") || text.contains("stack")) {
    ...
    if (text.contains("help") && (text.contains("q cli") || text.contains("capabilities") || text.contains("commands"))) ||
       text.contains("what can you do") || text.contains("show features") || text.contains("/help") {
        tools.push(ToolCall::Introspect(Introspect { query: None }));
    }
    
    // Detect knowledge tool requests
    if text.contains("remember") || text.contains("store") || text.contains("knowledge") ||
       text.contains("save context") || text.contains("search knowledge") {
    ...
    if text.contains("create todo") || text.contains("add task") || text.contains("new checklist") ||
        text.contains("track task") || text.contains("todo list") {
    ...
    if text.contains("think") || text.contains("analyze") || text.contains("reason") ||
       text.contains("step by step") || text.contains("complex") || text.contains("problem") {
    ...
    if trimmed.starts_with("/orchestrate") || trimmed.starts_with("/plan") || trimmed.starts_with("/multi") {
        tools.push(ToolCall::OrchestratedThinking(orchestrated_thinking::OrchestratedThinking::new(text)));
    }
    ...
    else if (text.contains("read file") || text.contains("show file") || text.contains("cat ") || 
        text.contains("list directory") || text.contains("ls ") || text.contains("dir ") ||
        text.starts_with("read ") || text.starts_with("show ") || text.starts_with("list ")) &&
       !text.contains("github.com") && !text.contains("TEST_SCENARIOS.md") {
        tools.push(ToolCall::FsRead(FsRead {
            operations: vec![fs_read::FsReadOperation::Directory(fs_read::FsDirectory {
                path: ".".to_string(),
    ...
    for line in lines {
        ...
        if is_executable_context && !trimmed.is_empty() && !trimmed.starts_with("#") {
            ...
            if trimmed.contains("├──") || trimmed.contains("└──") || trimmed.contains("│") || 
               ...
               trimmed.starts_with("Current Analysis:") || trimmed.starts_with("Repository Structure:") ||
               trimmed.starts_with("Found ") || trimmed.contains("appears to be") {
                continue;
            }
...
fn extract_file_path_from_text(text: &str) -> Option<(String, String)> {
    ...
    if let Some(search_op) = extract_search_operation(&text_lower) {
        return Some(("SEARCH".to_string(), format!("{}|{}", search_op.pattern, search_op.path)));
    }
    
    // Check for line range patterns
    if let Some(line_op) = extract_line_range_operation(&text_lower) {
        return Some(("LINES".to_string(), format!("{}|{}|{}", line_op.path, line_op.start_line, line_op.end_line)));
    }
        ...
        if let Ok(re) = regex::Regex::new(pattern) {
            if let Some(captures) = re.captures(&text_lower) {
                if let Some(file_match) = captures.get(1) {
                    let token = file_match.as_str().trim_matches(|c| c == '"' || c == '\'' || c == ',' || c == '.');
                    ...
                    if token.contains('.') || 
                       ["readme", "license", "changelog", "makefile", "dockerfile"].contains(&token) {
                        return Some(("FILE".to_string(), token.to_string()));
                    }
                }
            }
...
fn extract_simple_file_path(text: &str) -> Option<String> {
    ...
    for word in words {
        if word.contains('.') && (
            word.ends_with(".rs") || word.ends_with(".txt") || word.ends_with(".md") ||
            word.ends_with(".json") || word.ends_with(".toml") || word.ends_with(".py") ||
            word.ends_with(".js") || word.ends_with(".html") || word.ends_with(".css")
        ) {
...
fn extract_aws_command_from_text(text: &str) -> Option<UseAws> {
    ...
    // EC2 operations
    if text_lower.contains("ec2") && (text_lower.contains("list") || text_lower.contains("describe")) {
        return Some(UseAws {
            region: "ap-southeast-2".to_string(),
    ...
    // Lambda operations
    if text_lower.contains("lambda") && (text_lower.contains("list") || text_lower.contains("functions")) {
        return Some(UseAws {
            region: "ap-southeast-2".to_string(),
...
fn parse_execute_command(command_text: &str) -> Option<ExecuteCommand> {
    ...
    let valid_commands = [
        "ls", "pwd", "cd", "cat", "grep", "find", "ps", "top", "df", "free", "uptime", "whoami", "id", "date",
        "git", "docker", "kubectl", "aws", "az", "gcloud", "terraform", "npm", "yarn", "cargo", "make",
        "curl", "wget", "ssh", "scp", "rsync", "tar", "zip", "unzip", "chmod", "chown", "mkdir", "rmdir",
        "cp", "mv", "rm", "touch", "head", "tail", "sort", "uniq", "wc", "diff", "which", "whereis"
    ];
...
fn is_likely_filename(text: &str) -> bool {
    ...
    let file_extensions = [
        ".tf", ".md", ".txt", ".json", ".yaml", ".yml", ".toml", ".rs", ".py", ".js", ".ts",
        ".go", ".java", ".cpp", ".c", ".h", ".sh", ".bat", ".ps1", ".dockerfile", ".sql",
        ".html", ".css", ".xml", ".csv", ".log", ".conf", ".cfg", ".ini", ".env"
    ];
    
    // Check if it has a file extension
    if file_extensions.iter().any(|ext| text.to_lowercase().ends_with(ext)) {
        return true;
    }
    ...
    let common_files = [
        ...
        "rakefile", "procfile", ".gitignore", ".gitattributes", ".dockerignore"
    ];
...
fn print_custom_help() {
    ...
    println!();
    println!("{}", "Popular Subcommands              Usage: hello-ai [subcommand]".bright_white());
    println!("{}", "╭────────────────────────────────────────────────────╮".bright_cyan());
    println!("│ {}         {}                    │", "chat".bright_green().bold(), "Chat with Hello AI".white());
    println!("│ {}    {}│", "translate".bright_green().bold(), "Natural Language to Shell translation".white());
    println!("│ {}       {}             │", "doctor".bright_green().bold(), "Debug installation issues".white());
    println!("│ {}     {}       │", "settings".bright_green().bold(), "Customize appearance & behavior".white());
    println!("│ {}         {}                          │", "quit".bright_green().bold(), "Quit the app".white());
    println!("{}", "╰────────────────────────────────────────────────────╯".bright_cyan());
    println!();
    println!("{}", "To see all subcommands, use:".bright_white());
    println!(" {} {}", "❯".bright_blue(), "hello-ai --help-all".bright_cyan());
    println!("ㅤ");
}
...
async fn run_single_chat(message: String, app_config: Config) -> Result<(), Box<dyn std::error::Error>> {
    let config = aws_config::defaults(aws_config::BehaviorVersion::latest())
        .region(Region::new(app_config.cloud.aws_region.clone()))
        .load()
        .await;
...
async fn run_issue(title: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🐛".bright_blue(), "Create GitHub Issue".bright_blue().bold());
    println!();
...
async fn run_theme(name: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🎨".bright_blue(), "Q CLI Theme Management".bright_blue().bold());
    println!();
    ...
    match name {
        Some(theme_name) => {
            if available_themes.contains(&theme_name.as_str()) {
                ...
                let config_dir = std::env::var("HOME").unwrap_or_else(|_| ".".to_string()) + "/.config/q";
                std::fs::create_dir_all(&config_dir)?;
                std::fs::write(format!("{}/theme", config_dir), &theme_name)?;
                
                println!("✅ Theme '{}' applied successfully!", theme_name.bright_green());
            } else {
                println!("❌ Unknown theme: {}", theme_name.bright_red());
                println!("Available themes: {}", available_themes.join(", ").bright_blue());
            }
        }
        None => {
            // Show current theme and available options
            let config_file = std::env::var("HOME").unwrap_or_else(|_| ".".to_string()) + "/.config/q/theme";
            let current_theme = std::fs::read_to_string(&config_file)
            ...
            for theme in &available_themes {
                let indicator = if *theme == current_theme.trim() { "→" } else { " " };
                println!("  {} {}", indicator.bright_green(), theme.bright_white());
            }
            println!();
            println!("Usage: {} {}", "q theme <name>".bright_blue(), "# Set theme".bright_black());
        }
    }
...
async fn run_init(shell: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🐚".bright_blue(), "Q CLI Shell Integration".bright_blue().bold());
    println!();
    ...
    let (config_file, alias_content) = match shell_type.as_str() {
        "zsh" => ("~/.zshrc", "alias q='/Users/hans/Repo/hello-ai/qdev-sydney/q-bedrock/target/debug/q'"),
        "fish" => ("~/.config/fish/config.fish", "alias q='/Users/hans/Repo/hello-ai/qdev-sydney/q-bedrock/target/debug/q'"),
        _ => ("~/.bashrc", "alias q='/Users/hans/Repo/hello-ai/qdev-sydney/q-bedrock/target/debug/q'"),
    };
...
async fn run_diagnostic() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🔬".bright_green(), "Q CLI Diagnostic Tests".bright_green().bold());
    println!();
    ...
    println!("  • Tests Passed: {}", tests_passed.to_string().bright_green());
    println!("  • Tests Failed: {}", tests_failed.to_string().bright_red());
    
    if tests_failed == 0 {
        println!("\n{} All diagnostic tests passed!", "✅".bright_green());
    } else {
        println!("\n{} Some tests failed. Check configuration.", "⚠️".bright_yellow());
    }
...
async fn run_update() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🔄".bright_blue(), "Q CLI Update".bright_blue().bold());
    println!();
        ...
        .output() 
    {
        Ok(_) => {
            match std::process::Command::new("git")
                .args(&["rev-list", "--count", "HEAD..origin/feature/command-structure"])
                .current_dir("/Users/hans/Repo/hello-ai/qdev-sydney/q-bedrock")
                .output()
            {
                Ok(output) => {
                    ...
                    if commits == "0" {
                        ...
                        println!("     Run: {}", "git pull origin feature/command-structure".bright_blue());
...
async fn run_setup() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🔧".bright_blue(), "Q CLI Setup".bright_blue().bold());
    println!();
    ...
    match std::process::Command::new("aws").arg("--version").output() {
        ...
        Err(_) => {
            ...
            println!("     Install: {}", "https://aws.amazon.com/cli/".bright_blue());
        }
    }
    
    // Check AWS credentials
    println!("{} Checking AWS credentials...", "🔍".bright_blue());
    match std::process::Command::new("aws").args(&["sts", "get-caller-identity"]).output() {
        Ok(output) => {
    ...
    println!();
    println!("{} Setup completed! Run {} to start chatting.", "✅".bright_green(), "q chat".bright_blue());
    Ok(())
}

async fn run_debug() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🐛".bright_red(), "Q CLI Debug Information".bright_red().bold());
    println!();
    
    // System Information
    println!("{}", "SYSTEM INFORMATION:".bright_white().bold());
    println!("  • OS: {}", std::env::consts::OS.bright_green());
    println!("  • Architecture: {}", std::env::consts::ARCH.bright_green());
    ...
    let env_vars = [
        ...
        "NIGHTFALL_API_KEY", "SIMPLE_PII_SCAN", "HOME", "PATH"
    ];
        ...
        match std::env::var(var) {
            Ok(value) => {
                if var.contains("KEY") {
                    ...
                    println!("  • {}: {} entries", var.bright_blue(), value.split(':').count().to_string().bright_green());
                } else {
    ...
    println!("  • Current Directory: {}", current_dir.display().to_string().bright_green());
    ...
    println!("  • Bedrock Endpoint: {}", "bedrock-runtime.ap-southeast-2.amazonaws.com".bright_green());
    ...
    println!("  • Process ID: {}", std::process::id().to_string().bright_green());
        ...
        .output() 
    {
        if let Ok(memory_str) = String::from_utf8(output.stdout) {
            if let Ok(memory_kb) = memory_str.trim().parse::<u64>() {
                println!("  • Memory Usage: {} MB", (memory_kb / 1024).to_string().bright_green());
            }
        }
    }
    
    println!();
    println!("{} Debug information collected successfully!", "✅".bright_green());
    Ok(())
}

async fn run_translate(command: String, app_config: Config) -> Result<(), Box<dyn std::error::Error>> {
...
async fn run_doctor() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🩺".bright_green(), "Q CLI Doctor - Diagnostic Report".bright_green().bold());
    println!();
...
async fn run_settings(show: bool, set: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "⚙️".bright_blue(), "Q CLI Settings".bright_blue().bold());
    println!();
    ...
    if show || set.is_none() {
        ...
        println!("  • Region: {}", settings["region"].as_str().unwrap_or("ap-southeast-2").bright_cyan());
        println!("  • Theme: {}", settings["theme"].as_str().unwrap_or("auto").bright_yellow());
        println!("  • Verbose: {}", settings["verbose"].as_bool().unwrap_or(false).to_string().bright_green());
        println!("  • Auto-update: {}", settings["auto_update"].as_bool().unwrap_or(true).to_string().bright_green());
        println!("  • Shell integration: {}", settings["shell_integration"].as_bool().unwrap_or(true).to_string().bright_green());
    }
    ...
    if let Some(setting) = set {
        if let Some((key, value)) = setting.split_once('=') {
            match key {
                ...
                "shell_integration" => settings["shell_integration"] = serde_json::Value::Bool(value == "true"),
                _ => {
...
async fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🔐".bright_blue(), "AWS Login".bright_blue().bold());
    println!();
...
async fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🚪".bright_blue(), "AWS Logout".bright_blue().bold());
    println!();
...
async fn run_whoami() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "👤".bright_blue(), "Current User Information".bright_blue().bold());
    println!();
    ...
    if std::path::Path::new(&session_path).exists() {
        ...
        println!("  • User: {}", session["user_id"].as_str().unwrap_or("Unknown"));
        println!("  • Login time: {}", session["login_time"].as_str().unwrap_or("Unknown"));
        println!("  • Region: {}", session["region"].as_str().unwrap_or("ap-southeast-2"));
    } else {
...
async fn run_launch() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🚀".bright_blue(), "Launch Desktop App".bright_blue().bold());
    println!();
    println!("{}", "This is a CLI-only implementation.".bright_yellow());
    println!("{}", "Desktop app launch not available in this version.".bright_black());
    Ok(())
}

async fn run_profile() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "👤".bright_blue(), "User Profile".bright_blue().bold());
    println!();
...
async fn run_user() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "👥".bright_blue(), "User Management".bright_blue().bold());
    println!();
...
async fn run_integrations() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "🔗".bright_blue(), "System Integrations".bright_blue().bold());
    println!();
...
async fn check_bash_integration(home: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(content) = std::fs::read_to_string(&bashrc_path) {
        if content.contains("# Q CLI integration") {
            ...
            println!("✘ bash ~/.bashrc integration check: {} does not source post integration last", bashrc_path);
            println!("  Run q integrations install dotfiles to reinstall shell integrations for bash");
        }
    } else {
    ...
    if let Ok(content) = std::fs::read_to_string(&profile_path) {
        if content.contains("# Q CLI integration") {
            ...
            println!("✘ bash ~/.profile integration check: {} does not source pre integration", profile_path);
        }
    } else {
...
async fn check_zsh_integration(home: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(content) = std::fs::read_to_string(&zshrc_path) {
        if content.contains("# Q CLI integration") {
            ...
            println!("✘ zsh ~/.zshrc integration check: {} does not source Q CLI integration", zshrc_path);
        }
    } else {
...
async fn run_dashboard() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "📊".bright_blue(), "Dashboard".bright_blue().bold());
    println!();
    ...
    if let Err(_) = open_browser(url) {
        println!("{} {}", "⚠️".yellow(), "Could not open browser automatically".yellow());
    }
    println!("{} {}", "🌐".bright_green(), format!("Web GUI available at: {}", url).bright_green());
    ...
    match mcp_command {
                ...
                if let Ok(config) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(servers) = config["servers"].as_object() {
                        for (name, server) in servers {
                            ...
                            let status = if server["enabled"].as_bool().unwrap_or(true) { "enabled" } else { "disabled" };
        ...
        Some(McpCommands::Add) => {
            ...
            let mut config = if std::path::Path::new(&mcp_config_path).exists() {
                let content = std::fs::read_to_string(&mcp_config_path)?;
                serde_json::from_str(&content).unwrap_or_else(|_| serde_json::json!({"servers": {}}))
            } else {
        ...
        None => {
            println!("{} {}", "🔌".bright_blue(), "Model Context Protocol".bright_blue().bold());
            println!();
...
async fn run_inline() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} {}", "⚡".bright_blue(), "Inline Shell Completions".bright_blue().bold());
    println!();
    println!("{}", "Inline completion functionality".bright_yellow());
    println!("{}", "Use shell integration for completions.".bright_black());
    Ok(())
}
...
async fn run_agent(agent_command: Option<AgentCommands>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match agent_command {
                    ...
                    if let Ok(entry) = entry {
                        if entry.file_name().to_string_lossy().starts_with("agent-") && 
                           entry.path().extension().map_or(false, |ext| ext == "json") {
                            let name = entry.file_name().to_string_lossy().replace("agent-", "").replace(".json", "");
                            println!("  • {} (local)", name);
                        }
                    }
                }
        ...
        None => {
            println!("{} {}", "🤖".bright_blue(), "Agent Management".bright_blue().bold());
            println!();
...
async fn handle_save_conversation(history: &[serde_json::Value], name: Option<String>) {
    ...
    if let Err(e) = std::fs::create_dir_all(&conversations_dir) {
        println!("{} Failed to create conversations directory: {}", "Error:".bright_red(), e);
        return;
    }
    
    let filename = match name {
        Some(n) => format!("{}/{}.json", conversations_dir, n),
        None => format!("{}/conversation_{}.json", conversations_dir, chrono::Utc::now().timestamp()),
    };
    ...
    match std::fs::write(&filename, serde_json::to_string_pretty(&conversation_data).unwrap()) {
        Ok(_) => println!("{} Conversation saved to: {}", "✅".bright_green(), filename.bright_cyan()),
        Err(e) => println!("{} Failed to save conversation: {}", "Error:".bright_red(), e),
    }
}
...
async fn handle_compact_conversation(history: &mut Vec<serde_json::Value>) {
    if history.len() < 5 {
        println!("{} Conversation too short to compact", "Info:".bright_yellow());
        return;
    }
    
    let summary = format!("Summary of {} messages: Key topics discussed and important decisions made.", history.len());
    history.clear();
    history.push(json!({
        "role": "system",
...
async fn handle_agent_management() {
    ...
    println!();
    println!("{} Agent system not fully implemented in this version", "Info:".bright_yellow());
}

async fn handle_context_management() {
    println!("{} {}", "📄".bright_blue(), "Context File Management".bright_blue().bold());
    println!();
    
    let context_dir = ".amazonq/context";
    if let Err(e) = std::fs::create_dir_all(context_dir) {
        println!("{} Failed to create context directory: {}", "Error:".bright_red(), e);
        return;
    }
    ...
    if let Ok(entries) = std::fs::read_dir(context_dir) {
        ...
        if files.is_empty() {
            println!("{} No context files currently loaded", "Info:".bright_black());
        } else {
...
async fn handle_editor_prompt() -> Option<String> {
    println!("{} {}", "✏️".bright_blue(), "Opening editor for prompt composition...".bright_blue());
    
    let editor = std::env::var("EDITOR").unwrap_or_else(|_| "vi".to_string());
    let temp_file = format!("/tmp/q_prompt_{}.txt", chrono::Utc::now().timestamp());
    
    // Create temp file with instructions
    let instructions = "# Enter your prompt below this line\n# Lines starting with # will be ignored\n\n";
    ...
    match status {
        Ok(exit_status) if exit_status.success() => {
            ...
            if let Ok(content) = std::fs::read_to_string(&temp_file) {
                ...
                println!("{} Failed to read prompt from editor", "Error:".bright_red());
                None
            }
        }
        _ => {
            println!("{} Editor exited with error or was cancelled", "Error:".bright_red());
            let _ = std::fs::remove_file(&temp_file);
            None
        }
    }
...
async fn handle_reply_prompt(history: &[serde_json::Value]) -> Option<String> {
    println!("{} {}", "↩️".bright_blue(), "Opening editor with last assistant message quoted...".bright_blue());
    ...
    let quoted_content = format!("# Reply to the following message:\n# {}\n\n", 
...
async fn handle_prompts_management() {
    println!("{} {}", "📝".bright_blue(), "Custom Prompts Management".bright_blue().bold());
    println!();
    
    let prompts_dir = ".amazonq/prompts";
    if let Err(e) = std::fs::create_dir_all(prompts_dir) {
        println!("{} Failed to create prompts directory: {}", "Error:".bright_red(), e);
        return;
    }
...
async fn handle_experiment_toggle() {
    println!("{} {}", "🧪".bright_blue(), "Experimental Features".bright_blue().bold());
    println!();
    ...
    println!("  1. {} - Context usage percentage in prompt", 
        if experiments["context_usage"].as_bool().unwrap_or(false) { "ON".bright_green() } else { "OFF".bright_red() });
    println!("  2. {} - Enhanced thinking mode", 
        if experiments["thinking_mode"].as_bool().unwrap_or(false) { "ON".bright_green() } else { "OFF".bright_red() });
    println!("  3. {} - Tangent mode for conversations", 
        if experiments["tangent_mode"].as_bool().unwrap_or(false) { "ON".bright_green() } else { "OFF".bright_red() });
    println!("  4. {} - Advanced checkpointing", 
        if experiments["checkpointing"].as_bool().unwrap_or(false) { "ON".bright_green() } else { "OFF".bright_red() });
    println!();
...
async fn handle_changelog_display() {
    println!("{} {}", "📋".bright_blue(), "Q CLI Changelog".bright_blue().bold());
    println!();
...
async fn handle_usage_display(history: &[serde_json::Value]) {
    println!("{} {}", "📊".bright_blue(), "Context Window Usage".bright_blue().bold());
    println!();
    ...
    println!("{} Messages in conversation: {}", "📝".bright_cyan(), history.len().to_string().bright_white());
    println!("{} Total characters: {}", "🔤".bright_cyan(), total_chars.to_string().bright_white());
    println!("{} Estimated tokens: {}", "🎯".bright_cyan(), estimated_tokens.to_string().bright_white());
    println!("{} Context usage: {}%", "📊".bright_cyan(), 
    ...
    if usage_percent > 80.0 {
        println!();
        println!("{} {} Consider using /compact to summarize the conversation", 
            "⚠️".bright_yellow(), "High usage detected!".bright_yellow());
    }
}

async fn handle_mcp_management() {
    println!("{} {}", "🔌".bright_blue(), "MCP Server Management".bright_blue().bold());
    println!();
    
    let mut mcp_manager = McpManager::new();
    if let Err(e) = mcp_manager.load_servers().await {
        println!("{} Failed to load MCP servers: {}", "Error:".bright_red(), e);
        return;
    }
...
async fn handle_hooks_management() {
    println!("{} {}", "🪝".bright_blue(), "Hooks Management".bright_blue().bold());
    println!();
...
fn display_comprehensive_help() {
    println!("{}", "📚 Q CLI - Complete Command Reference".bright_cyan().bold());
    println!();
...
async fn handle_model_selection() {
    ...
    println!("  1. {} - Claude 3.5 Sonnet (Default)", "anthropic.claude-3-5-sonnet-20241022-v2:0".bright_cyan());
    println!("  2. {} - Claude 3 Haiku", "anthropic.claude-3-haiku-20240307-v1:0".bright_cyan());
    println!("  3. {} - Claude 3 Opus", "anthropic.claude-3-opus-20240229-v1:0".bright_cyan());
    println!();
    ...
    println!("{} Current model: {}", "🎯".bright_green(), current_model.bright_white());
    println!();
    println!("{} All models use Sydney Bedrock (ap-southeast-2)", "🇦🇺".bright_green());
}
