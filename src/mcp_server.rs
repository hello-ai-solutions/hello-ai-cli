...
impl McpManager {
                ...
                for (name, server_config) in servers {
                    if let (Some(command), Some(args)) = (
                        ...
                        let server = McpServer {
                            ...
                            args: args.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect(),
    ...
    #[allow(dead_code)]
    pub async fn add_server(&mut self, name: String, command: String, args: Vec<String>) -> Result<String, Box<dyn std::error::Error>> {
        let server = McpServer {
            name: name.clone(),
            command,
            args,
