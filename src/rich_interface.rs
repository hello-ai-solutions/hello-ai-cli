...
impl AmazonQTheme {
    ...
    pub fn format_code(text: &str) -> String {
        text.truecolor(248, 249, 250).on_truecolor(35, 47, 62).to_string()
    }
}
...
impl RichInterface {
    ...
    pub fn create_interactive_example(&self, code: &str, language: &str, explanation: &str) -> String {
        format!(
            "{}\n\n{}\n\n{}\n\n{}",
            format!("💡 {}", "Interactive Code Example").truecolor(255, 153, 0).bold(),
            self.format_code_response(code, language),
            format!("📖 {}", "Explanation").truecolor(0, 123, 255).bold(),
    ...
    pub fn create_file_tree(&self, files: &[String]) -> String {
        let mut tree = format!("{}\n", 
            format!("📁 {}", "Project Structure").truecolor(255, 153, 0).bold()
        );
    ...
    pub fn create_diff_visualization(&self, old_code: &str, new_code: &str) -> String {
        let mut diff = format!("{}\n", 
            format!("🔄 {}", "Code Changes").truecolor(255, 153, 0).bold()
        );
    ...
    pub fn create_command_palette(&self, commands: &[(&str, &str)]) -> String {
        let mut palette = format!("{}\n", 
            format!("⌨️  {}", "Command Palette").truecolor(255, 153, 0).bold()
        );
    ...
    pub fn format_pii_detection(&self, pii_found: bool, details: &str, is_nightfall: bool) -> String {
        if !pii_found {
            return String::new();
        }
        ...
        content.lines()
            .map(|line| {
                if line.trim_start().starts_with("- ") {
                    ...
                    format!("{}{} {}", " ".repeat(indent), bullet, line.trim_start().strip_prefix("- ").unwrap())
                } else {
                    line.to_string()
                }
            })
