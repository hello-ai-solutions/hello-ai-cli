...
impl SecurityScanner {
    ...
    pub async fn scan_file(&self, file_path: &str) -> Result<Vec<SecurityIssue>, Box<dyn std::error::Error>> {
    ...
    pub async fn scan_directory(&self, dir_path: &str) -> Result<Vec<(String, Vec<SecurityIssue>)>, Box<dyn std::error::Error>> {
        ...
        for entry in fs::read_dir(dir_path)? {
            ...
            if path.is_file() {
                if let Some(ext) = path.extension() {
                    if matches!(ext.to_str(), Some("rs") | Some("py") | Some("js") | Some("ts") | Some("java")) {
