...
async fn get_ai_response(
    ...
    for msg in history.iter().rev().take(10).rev() { // Last 10 messages for context
    ...
    println!("Sending request to Bedrock with {} messages", messages.len());
    
    let response = client
        .invoke_model()
        .model_id("anthropic.claude-3-5-sonnet-20241022-v2:0")
        .content_type("application/json")
        .body(aws_sdk_bedrockruntime::primitives::Blob::new(request_body.to_string()))
        .send()
        .await
        .map_err(|e| {
